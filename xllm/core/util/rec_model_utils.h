...
inline bool use_legacy_onerec_prefill_only_contract(
    const RecRuntimeConfig& runtime_config) {
  return runtime_config.enable_rec_prefill_only;
}

inline bool is_onerec_xattention_mode(const RecRuntimeConfig& runtime_config) {
  return !use_legacy_onerec_prefill_only_contract(runtime_config) &&
         runtime_config.max_decode_rounds > 0;
}

inline bool use_legacy_onerec_prefill_only_contract() {
  return get_rec_runtime_enable_prefill_only();
}
...
inline RecPipelineType get_rec_pipeline_type(
  ...
  switch (kind) {
    ...
    case RecModelKind::kOneRec:
      return is_onerec_xattention_mode(runtime_config)
