...
void apply_onerec_pipeline_toggles(xllm::Options* options,
  ...
  CHECK(runtime_config != nullptr);
  const bool enable_prefill_only = runtime_config->enable_rec_prefill_only;
  const bool enable_onerec_xattention =
      !enable_prefill_only && runtime_config->max_decode_rounds > 0;
  FLAGS_enable_rec_prefill_only = enable_prefill_only;
  FLAGS_max_decode_rounds = runtime_config->max_decode_rounds;
