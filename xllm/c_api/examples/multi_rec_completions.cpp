...
// 并发示例用于功能验证，不做无限压测；每次成功请求后短暂停顿，避免把
// compile_and_run.sh 变成持续高压 benchmark。
const int32_t kConcurrentRequestPauseMs = 100;
...
bool init_rec_handler_for_chip(int32_t chip_index,
  ...
  // 默认 ./Logs 在容器内常不存在，glog 会报错；/tmp 一般可写。
...
void concurrent_infer_loop(XLLM_REC_Handler* handler,
  ...
  while (std::chrono::steady_clock::now() < t_end) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(kConcurrentRequestPauseMs));
